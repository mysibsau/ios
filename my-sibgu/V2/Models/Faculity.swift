...
// MARK: - Request
struct FaculityRequest: Request, RequestWithDefaultQueryParams, RequestWithDefaultHeaderParams {
    typealias Response = [Faculity]
    
    var apiVersion: RequestModel.Version { .v3 }
    var path: String { "/campus/faculties/" }
}

// MARK: - Model
struct Faculity: Decodable {
...
extension Faculity: Equatable { }

extension Faculity: Storable {
    
    var storeModel: RFaculity {
        let rAA = RFaculity()
        rAA.id = id
        rAA.name = name
        rAA.logo = logoString
...
// MARK: - RModel
class RFaculity: Object, ConvertableToApp {
